...
const tracker = new Map(), cooldown = new Set();
...
async function handleAntiSpam(api, guildId, message) {
  ...
  if (cfg.antispam_enabled && recent.length >= cfg.antispam_max_msgs) {
    cooldown.add(key); setTimeout(() => cooldown.delete(key), cfg.antispam_interval * 2);
    await punish(api, guildId, message, cfg, `[AntiSpam] ${recent.length} mesaje în ${cfg.antispam_interval/1000}s`);
    return;
  }
  if (cfg.antiflood_enabled) {
    ...
    if (c.length > 0 && recent.filter(m => m.content === c).length >= cfg.antiflood_duplicates) {
      cooldown.add(key); setTimeout(() => cooldown.delete(key), cfg.antispam_interval * 2);
      await punish(api, guildId, message, cfg, `[AntiFlood] Același mesaj repetat`);
    }
  }
}
...
async function punish(api, guildId, message, cfg, reason) {
  ...
  try {
    ...
    try { const dm = await api.users.createDM(user.id); await api.channels.createMessage(dm.id, { content: `Acțiune: ${cfg.antispam_action.toUpperCase()}\nMotiv: ${reason}` }); } catch (_) {}
    const entry = await createCase(guildId, { action: cfg.antispam_action.toUpperCase(), userId: user.id, userTag: user.username, modId: 'bot', modTag: 'FluxerGuard', reason, duration: cfg.antispam_action === 'timeout' ? formatMs(cfg.antispam_timeout_ms) : null, auto: true });
    await sendLog(api, guildId, 'ANTISPAM', { 'User': `${user.username} (${user.id})`, 'Acțiune': cfg.antispam_action.toUpperCase(), 'Case': entry.caseId }, entry);
